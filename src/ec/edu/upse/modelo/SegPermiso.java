...
@NamedQueries({
	...
	@NamedQuery(name="SegPermiso.buscarPadrePorPerfil", query="SELECT s FROM SegPermiso s where s.segPerfil.idPerfil = :idperfil and s.estado = 'A' and s.segMenu.idMenuPadre = 0"),
	@NamedQuery(name="SegPermiso.buscarTodosPorPerfil", query="SELECT s FROM SegPermiso s where s.segPerfil.idPerfil = :idperfil and s.estado = 'A'"),
})	
